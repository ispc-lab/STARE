...
def point2d_collector(
    ...
    os.close(fd)
    unpack = struct.Struct("<QffI")

    last_seq = 0
    while not stop_event.is_set():
        buf.seek(0)
        seq, x, y, _ = unpack.unpack(buf.read(unpack.size))
...
def arm_follow_target(
    ...
    print(robot.rm_set_arm_max_line_acc(3.2))

    print(robot.rm_get_arm_max_angular_acc())
    print(robot.rm_set_arm_max_angular_acc(8.0))
