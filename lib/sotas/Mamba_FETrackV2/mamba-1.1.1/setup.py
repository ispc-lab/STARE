...
class CachedWheelsCommand(_bdist_wheel):
    ...
    def run(self):
        if FORCE_BUILD:
            print("MAMBA_FORCE_BUILD=TRUE is set. Building from source...")
            return super().run()
        ...
        # 1. 检查本地是否存在匹配的 wheel 文件
        if os.path.exists(wheel_filename):
            print(f"Found local precompiled wheel: {wheel_filename}")
        else:
            # 2. 如果本地不存在，才尝试从网络下载
            print("Local wheel not found. Guessing wheel URL: ", wheel_url)
            try:
                urllib.request.urlretrieve(wheel_url, wheel_filename)
            except urllib.error.HTTPError:
                # 3. 如果下载也失败了，才回退到从源码编译
                print("Precompiled wheel not found online. Building from source...")
                super().run()
                return  # 从源码构建后，直接退出，避免执行后续的文件移动代码
            except Exception as e:
                # 捕获其他可能的网络错误，比如您遇到的下载不完整
                print(f"Failed to download wheel due to an error: {e}")
        ...
        # 如果能执行到这里，说明 wheel_filename 已经成功下载或在本地找到
        # Make the archive
        ...
        print("Moving wheel to dist folder:", wheel_path)
