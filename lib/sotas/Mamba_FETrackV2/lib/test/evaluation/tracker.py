...
class Tracker:
    ...
    def run_sequence(self, seq, stream_setting=None, debug=None, visualization=None, visdom_info=None):
        ...
        if isinstance(seq.init_info(), tuple):
            init_info, start_frame_idx = seq.init_info()
        else:
    ...
    def _track_sequence(self, tracker, seq, start_frame_idx, init_info):
        ...
        ##########################################################################
        def search_rgb_frames(frames, last_idx, curr_t):
            for idx in range(last_idx, len(frames)):
                if frames[idx][0] > curr_t:
                    return idx - 1

            return len(frames) - 1

        ae_file = seq.rgb_frame_info['aeFile']
        with AedatFile(ae_file) as f:
            events = np.hstack([packet for packet in f['events'].numpy()])
            start_time = events['timestamp'][0]
            events['timestamp'] = events['timestamp'] - start_time
            frames = [[frame.timestamp, frame.image] for frame in f['frames']]  # for Mamba-FETrackV2
            frames = [[frame[0] - start_time, frame[1]] for frame in frames]
            frames[0][0] = 0

        t_anno = seq.rgb_frame_info['t_anno']
        window = seq.rgb_frame_info['window']
        fps = seq.rgb_frame_info['fps']
        multiple = 500 // fps
        t_anno = t_anno[::multiple]

        rgb_frame_idx_list = []
        curr_idx = 0
        for idx in range(len(t_anno)):
            ...
            curr_idx = search_rgb_frames(frames, curr_idx, label_time)
            rgb_frame_idx_list.append(curr_idx)

        assert len(rgb_frame_idx_list) == len(seq.frames)
        ###########################################################################
                    
        # Initialize
        event_image = self._read_image(seq.frames[start_frame_idx])
        image = cv.cvtColor(frames[start_frame_idx][1], cv.COLOR_BGR2RGB)
        ...
        for frame_num, (frame_path, rgb_frame_idx) in enumerate(zip(seq.frames[start_frame_idx+1:],  rgb_frame_idx_list[start_frame_idx+1:]), start=start_frame_idx+1):
            event_img = self._read_image(frame_path)
            image = cv.cvtColor(frames[rgb_frame_idx][1], cv.COLOR_BGR2RGB)
    ...
    def _track_evstream(self, tracker, seq, init_info, stream_setting, **kwargs):
        ...
        ##########################################################################
        def search_frames(frames, last_idx, curr_t):
            for idx in range(last_idx, len(frames)):
                if frames[idx][0] > curr_t:
                    return idx - 1
        ...
        event_rep = cv.cvtColor(event_rep, cv.COLOR_BGR2RGB)

        frame_idx = search_frames(frames, frame_idx, t_template)
        rgb_frame = cv.cvtColor(frames[frame_idx][1], cv.COLOR_BGR2RGB)
        ...
        out = tracker.initialize(rgb_frame, event_rep, frame_idx, init_info)
        if out is None:
            out = {}
        ...
        while True:
            ...
            frame_idx = search_frames(frames, frame_idx, t_right)
            rgb_frame = cv.cvtColor(frames[frame_idx][1], cv.COLOR_BGR2RGB)
            ...
            out = tracker.track(rgb_frame, event_rep, info)
            prev_output = OrderedDict(out)
            torch.cuda.synchronize()
