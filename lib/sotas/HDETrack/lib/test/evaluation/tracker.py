...
class Tracker:
    ...
    def run_sequence(self, seq, stream_setting=None, debug=None, visualization=None, visdom_info=None):
        ...
        if seq.dataset in ['esot500s', 'esot500hs']:
            output = self._track_evstream(tracker, seq, start_frame_idx, init_info, stream_setting)
        else:
            output = self._track_sequence(tracker, seq, start_frame_idx, init_info)
    ...
    def _track_evstream(self, tracker, seq, start_frame_idx, init_info, stream_setting, **kwargs):
        """
        Core Function
        stream_setting:
        ...
        if not stream_setting.convert_time:
            ...
            out = tracker.initialize(event_rep, start_frame_idx, init_info)
            if out is None:
                out = {}
            prev_output = OrderedDict(out)
            ...
            out = tracker.initialize(event_img, start_frame_idx, init_info)
            if out is None:
                out = {}
            prev_output = OrderedDict(out)
            torch.cuda.synchronize()
